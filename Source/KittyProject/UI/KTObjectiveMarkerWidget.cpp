...
void UKTObjectiveMarkerWidget::SetTargetActor(AActor* NewTargetActor)
{
...
void UKTObjectiveMarkerWidget::UpdateMarker()
{
    ...
    const FVector TargetLocation = TargetActor->GetActorLocation() + FVector(0.0f,0.0f,50.0f);
    ...
    else
    {
        ...
        const float XScale =
            FMath::Abs(Direction2D.X) > KINDA_SMALL_NUMBER
                ? HalfSize.X / FMath::Abs(Direction2D.X)
                : BIG_NUMBER;

        const float YScale =
            FMath::Abs(Direction2D.Y) > KINDA_SMALL_NUMBER
        ...
        ArrowAngle = FMath::RadiansToDegrees(AngleRadians);
    }

    if (Img_Arrow)
    {
        Img_Arrow->SetRenderTransformAngle(ArrowAngle);
    }
